...
export function QuestionBankEditor({ jobId }: { jobId: number }) {
  ...
  return (
    <div className="relative">
      ...
      {open && (
        <div className="absolute right-0 z-20 mt-2 max-h-[70vh] w-[28rem] overflow-y-auto rounded-xl border border-border bg-popover p-4 shadow-xl">
              ...
              {goals.map((g) => (
                <div key={g.id} className="rounded-lg border border-border p-3">
                  <input
                    ...
                    className="mb-2 w-full rounded-md border border-border bg-background px-2 py-1.5 text-sm font-medium focus:outline-none focus:ring-1 focus:ring-primary"
                  />
